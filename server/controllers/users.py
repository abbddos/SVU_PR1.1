...
ALLOWED_ROLES = ['root','admin', 'manager', 'user']
...
@users_bp.route('/', methods = ['POST'], endpoint='create_user_endpoint')
@jwt_required()
def create_user():
...
@users_bp.route('/all', methods = ['GET'], endpoint='get_all_users_endpoint')
@jwt_required()
...
@users_bp.route('/admins', methods = ['GET'], endpoint = 'get_admins_endpoint')
def get_admins():
    users = User.query.filter_by(role="admin").order_by(User.created_at.desc()).all()
    return jsonify([user.serialize() for user in users]), 200

@users_bp.route('/profile', methods = ['GET'], endpoint='get_profile_endpoint')
@jwt_required()
def get_user_by_id():
...
@users_bp.route('/<int:user_id>', methods=['PUT'], endpoint='update_user_endpoint')
@jwt_required()
...
@users_bp.route('/change_password', methods = ['PUT'], endpoint='change_password_endpoint')
@jwt_required()
...
@users_bp.route('/reset_password/<int:user_id>', methods = ['PUT'], endpoint='reset_password_endpoint')
@jwt_required()
def reset_password(user_id):
    ...
    try:
        reset_pass = generate_random_password(user.email)
        user.set_password(reset_pass)
        db.session.commit()
        cache.clear()
...
@users_bp.route('/<int:user_id>/status', methods = ['PATCH'], endpoint='toggle_status_endpoint')
@jwt_required()
...
@users_bp.route('/<int:user_id>', methods=['DELETE'], endpoint='delete_user_endpoint')
@jwt_required()
